...
def store_mfcc_npy(fname):
    y, sr = librosa.load('spoken_numbers_pcm_ch/'+fname)
    ...
    mfcc_final = cv2.resize(mfcc.astype('float'), (30, 13), interpolation=cv2.INTER_LINEAR)
    np.save('mfcc_npy_ch/'+fname, mfcc_final)

filename = glob.glob('spoken_numbers_pcm_ch/**.wav')
...
y_list = []
t_list = []
filename_npy = glob.glob('mfcc_npy_ch/**.npy')
...
x_train = np.asarray(x_list[:1350])
y_train = np.asarray(y_list[:1350])
x_test = np.asarray(x_list[1350:])
y_test = np.asarray(y_list[1350:])
...
model = Sequential()
model.add(LSTM(units=128,return_sequences=True,recurrent_initializer='orthogonal',input_shape=(13, 30)))
